...
def _weeks_payload(week_nums: list[int]) -> dict:
    ...
    return {
        "weeks": [{"week": n, "weekType": "REG", "season": 2024} for n in week_nums]
    }
...
def _weekly_details_payload(game_ids: list[str], week: int = 1) -> list:
    ...
    return [
        {"id": gid, "week": week, "season": 2024, "homeTeam": {}, "awayTeam": {}}
        for gid in game_ids
    ]
...
class TestListSeasonWeeks:
    def test_returns_sorted_week_numbers(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod,
            "_list_weeks",
            lambda season, season_type, headers=None: _weeks_payload([3, 1, 2]),
        )
    ...
    def test_eighteen_regular_season_weeks(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod,
            "_list_weeks",
            lambda season, season_type, headers=None: _weeks_payload(
                list(range(1, 19))
            ),
        )
    ...
    def test_postseason_weeks(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod,
            "_list_weeks",
            lambda season, season_type, headers=None: _weeks_payload([1, 2, 3, 4]),
        )
    ...
    def test_empty_weeks_returns_empty_list(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod, "_list_weeks", lambda season, season_type, headers=None: {"weeks": []}
        )
    ...
    def test_weeks_missing_key_returns_empty_list(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mod, "_list_weeks", lambda season, season_type, headers=None: {}
        )
        assert list_season_weeks(2024, "REG") == []
...
class TestBuildRawLibrary:
    def _setup(self, monkeypatch, week_nums: list[int], game_ids: list[str]):
        ...
        monkeypatch.setattr(
            mod,
            "_fetch_weekly_details",
            lambda season, season_type, week, headers=None: _weekly_details_payload(
                game_ids, week
            ),
        )
    ...
    def test_resume_skips_existing_files(self, tmp_path, monkeypatch):
        ...
        build_raw_library(
            [2024], output_dir=tmp_path, season_types=["REG"], resume=True
        )
    ...
    def test_no_resume_overwrites_existing(self, tmp_path, monkeypatch):
        ...
        build_raw_library(
            [2024], output_dir=tmp_path, season_types=["REG"], resume=False
        )
...
class TestLoadWeeklyRaw:
    ...
    def test_round_trip_list_payload(self, tmp_path):
        from python.nfl_raw_scrape.raw_fetcher import load_weekly_raw
...
class TestListLibraryFiles:
    ...
    def test_finds_all_week_files(self, tmp_path):
        from python.nfl_raw_scrape.raw_fetcher import list_library_files
...
class TestNflverseGameId:
    ...
    def test_preseason_never_collides_with_postseason(self):
        # PRE weeks 1-3 used to take the postseason offset (19-21): a Wild Card
        # rematch of a preseason game raised the duplicate-id error on extraction.
