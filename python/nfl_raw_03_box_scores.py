...
def _commit(out_dir: str, season: int, n: int) -> None:
    ...
    if (
        subprocess.run(
            ["git", "diff", "--cached", "--quiet", "--", path], check=False
        ).returncode
        == 0
    ):
        ...
        return
    subprocess.run(
        ["git", "commit", "-q", "-m", f"NFL Raw: {season} box scores ({n} games)"],
        check=True,
    )
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--delay",
        type=float,
        default=float(os.environ.get("NFL_BOX_SCORE_DELAY", "0.5")),
        help="seconds after each request (env NFL_BOX_SCORE_DELAY; default 0.5)",
    )
    ap.add_argument("--rescrape", action="store_true")
    ap.add_argument(
        "--limit", type=int, default=0, help="cap new games per season (smoke tests)"
    )
    ap.add_argument(
        "--commit", action="store_true", help="git commit each season's box scores"
    )
