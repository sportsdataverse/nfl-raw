...
#
# Resolution order:
#   1. $SDV_VENV_PYTHON / $NFL_VENV_PYTHON   -- explicit override, always wins
...
#
# NFL_VENV_PYTHON is accepted as a repo-local alias. Both names are honoured,
# SDV_VENV_PYTHON first.
#
...
#
# The bootstrap at (3) matters here: a venv sweep on 2026-08-12 removed .venv
# from nearly every repo on this box, and cron's PATH had no /root/.local/bin,
# so resolution fell through to an ancient ambient python and every scraper
# died at import for days without anyone noticing.
#
...
_sdv_resolve() {
  ...
  elif [ -n "${NFL_VENV_PYTHON:-}" ]; then
    SDV_PY="$NFL_VENV_PYTHON"
...
if [ -n "$SDV_PY" ] && [ -z "${SDV_VENV_PYTHON:-}${NFL_VENV_PYTHON:-}" ] \
...
if [ -z "$SDV_PY" ] || [ ! -x "$SDV_PY" ]; then
  ...
  echo "       Tried \$SDV_VENV_PYTHON, \$NFL_VENV_PYTHON, $_sdv_repo/.venv," >&2
